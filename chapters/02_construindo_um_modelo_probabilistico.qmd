---
title: "Formalização - Parte 1 de 2 - Espaço de Probabilidade"
---
...
\left.
\begin{aligned}
\text{1. Quais resultados um ensaio do experimento pode produzir?}
&\quad\longrightarrow\quad \Omega \\[0.35em]

\text{2. Quais conjuntos desses resultados vamos admitir como eventos?}
&\quad\longrightarrow\quad \mathcal F \\[0.35em]

\text{3. Como atribuímos uma probabilidade a cada um desses eventos?}
...
Assim, podemos pensar em um mesmo experimento sendo realizado em vários ensaios: $\text{Ensaio}_1,\text{Ensaio}_2,\text{Ensaio}_3,\ldots$
...
Se lançamos uma moeda três vezes e nosso experimento foi definido como:

> lançar uma moeda três vezes e observar toda a sequência,

então $HTH$ é **um resultado de um único experimento**.

Os três lançamentos não precisam ser tratados como três experimentos diferentes.

Tudo depende de como definimos o experimento que queremos modelar.
...
No lançamento de uma moeda, $\Omega=\{H,T\}$.

Cada elemento de $\Omega$ representa um possível resultado do experimento.

Costumamos representar um resultado particular por uma letra minúscula grega, $\omega$.

Portanto, $\omega \in \Omega$.
...
Mas antes do lançamento nosso modelo precisava contemplar todos os resultados: $1,2,3,4,5,6$.
...
Um resultado precisa informar tanto o primeiro quanto o segundo dado.

Assim, $\omega=(i,j)$, onde $i,j\in\{1,2,3,4,5,6\}$.

O espaço amostral contém, portanto,
...
Existem $6\times6=36$ resultados possíveis.
...
Agora imagine que nossa pergunta seja um pouco mais detalhada:

> **Se houver uma compra, qual será a forma de pagamento utilizada?**

Nesse caso, precisamos distinguir mais resultados possíveis. Podemos definir, por exemplo,
...
O que mudou foi **a pergunta que queremos responder** e, consequentemente, nossa **representação matemática** do fenômeno.
...
$Y=1$ para churn e $Y=0$ para não churn.
...
Quais são todos os resultados possíveis? $\Omega=\{HH,HT,TH,TT\}$.

Por que precisamos tanto de $HT$ quanto de $TH$?

Porque eles representam resultados diferentes:

- $HT$: primeiro saiu cara e depois coroa;
- $TH$: primeiro saiu coroa e depois cara.

Se definíssemos $\Omega=\{HH,HT,TT\}$, o espaço amostral estaria incompleto.
...
A exaustividade funciona como uma checagem de sanidade do modelo: **existe alguma coisa que pode acontecer no mundo, mas que nosso espaço amostral esqueceu de representar?**

No exemplo das moedas, esquecer $TH$ produz exatamente esse problema.
...
::: {.callout-note title="Mas HT e TH precisam sempre ser separados?"}
...
então $HT$ e $TH$ são resultados diferentes e precisamos de $\Omega=\{HH,HT,TH,TT\}$.
...
Os possíveis resultados são simplesmente $\Omega=\{0,1,2\}$.

Tanto $HT$ quanto $TH$ correspondem ao mesmo resultado: **uma cara**.
...
No experimento em que registramos a sequência das duas moedas, se observamos $HT$, não observamos simultaneamente $HH$, $TH$ ou $TT$.
...
O terceiro não é válido se estivermos registrando a sequência completa, pois deixou de fora o resultado possível $TH$.
:::
...
Se conseguimos perguntar sobre $A$ e $B$, também queremos conseguir perguntar sobre **não $A$**, **$A$ ou $B$**, **$A$ e $B$**.

Por isso queremos que esses novos eventos **também** continuem pertencendo à mesma família.

A matemática chama essa propriedade de **fechamento**: aplicamos determinadas operações a elementos da coleção e o resultado continua dentro dela. (_É abstrato, mas faz todo sentido, veja abaixo!_)

Por exemplo:

- $A\in\mathcal F \Rightarrow A^c\in\mathcal F$. 
    Se $A\in\mathcal F$, queremos também que $A^c\in\mathcal F$, pois, se conseguimos perguntar **"A ocorreu?"**, também queremos conseguir perguntar **"A não ocorreu?"**.

- Da mesma forma, se $A,B\in\mathcal F$, queremos que $A\cup B\in\mathcal F$, pois, se conseguimos perguntar sobre $A$ e sobre $B$, também queremos conseguir perguntar **"A ou B ocorreu?"**.
...
Em espaços finitos, frequentemente podemos trabalhar dessa forma, pensando que $\mathcal F$ pode "facilmente" tomado como o **conjunto das partes**:  $\mathcal F=2^\Omega$.

Porém, como vimos mais acima no estudo do [**Espaço Amostral**]{.conceito}, a maioria das perguntas do mundo real cai no caso de [**espaços infinitos**]{.conceito}, por exemplo: _quanto tempo um usuário leva para concluir o checkout?_

Em espaços infinitos — principalmente os contínuos (_lembra?_) — existem algumas sutilezas e nem sempre consideramos todos os subconjuntos de $\Omega$ como eventos. Não precisamos entrar nisso agora.
